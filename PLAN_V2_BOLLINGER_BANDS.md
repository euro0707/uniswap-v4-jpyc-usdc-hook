...
- **上部バンド**: MA + (k × σ)
- **下部バンド**: MA - (k × σ)（**アンダーフロー対策**: `k × σ >= MA` の場合は `lowerBand = 0` にクランプし、BASE_FEEにフォールバック）
- **パラメータ**:
  - 期間（N）: 20観測（デフォルト）
...
**丸め対策**: `bandWidth * 0.1` の整数演算で0に丸められるケースを防ぐため、`FullMath.mulDiv(bandWidth, 1000, 10000)` で固定小数点のまま計算し、最小閾値（例: bandWidth の 1bps = bandWidth / 10000）を設ける。
...
contract BollingerBandsDynamicFeeHook is BaseHook, Ownable, Pausable {
    ...
    uint256 public constant MIN_UPDATE_INTERVAL = 1 hours; // 更新間隔（基本）
    uint256 public constant MAX_STALENESS = 2 hours;     // Staleness上限（超過時はMID_FEE以上に引き上げ）
    uint256 public constant CIRCUIT_BREAKER_STALENESS = 6 hours; // CB発動閾値
...
library BollingerBandsLibrary {
    ...
    //
    // **オーバーフロー対策**: 統計計算（標準偏差）では priceX96 を直接使用せず、
    // 以下のいずれかの方法でスケーリング:
    // (1) tick/log価格（int24）で統計計算を実施（推奨）、またはconst PRICE_SCALE = 1e12; で priceX96 / PRICE_SCALE に縮小
    // (2) 分散計算時は FullMath.mulDiv() で 512bit 演算を活用し、オーバーフロー検知
    // (3) 極端な価格差（差分の二乗が 2^256 超過リスク）を検出して BASE_FEE にフォールバック
...
2. **統計的異常検出（二段階計算方式）**
   - **第1段階**: 全観測から仮の平均・標準偏差を計算
   - **第2段階**: 3σを超える観測を除外し、残りで再計算（平均・標準偏差）
   - **フィルタ後の観測数不足（< 20観測）**: BASE_FEE（0.03%）にフォールバック
   - Z-scoreベースの外れ値検出

3. **バンド計算の健全性チェック**
   - **アンダーフロー防止**: 下部バンド計算前に `k × σ >= MA` を検出し、該当時は `lowerBand = 0` にクランプしてBASE_FEE（0.03%）にフォールバック
...
4. **観測数の最小要件（厳格化）**
   ...
   - **Staleness段階制御**:
     - 2時間超過（MAX_STALENESS）: 手数料をMID_FEE（0.2%）以上に引き上げ
     - 6時間超過（CIRCUIT_BREAKER_STALENESS）: サーキットブレーカー発動、プール一時停止
...
- ✅ **Staleness段階制御テスト（2h超過でMID_FEE以上、6h超過でCB発動）**
...
### 課題1: 標準偏差計算の精度とガスコスト（オーバーフロー対策含む）
- **問題**: オンチェーンでの平方根計算は高コスト、かつ priceX96 の差分二乗が 256bit を超えてオーバーフローする可能性
- **対策**:
  - **スケーリング方式の選択**:
    - 方式A（推奨）: tick/log価格（int24）で統計計算を実施し、オーバーフローリスクを根本的に回避
    - 方式B: priceX96 を const PRICE_SCALE = 1e12 で縮小してから統計計算
  - **オーバーフロー検知**: 分散計算時に FullMath.mulDiv() で 512bit 演算を活用し、極端な価格差（差分二乗が 2^256 超過）を検出してBASE_FEEにフォールバック
...
### 課題4: リアルタイム性と更新頻度（Staleness対策）
- **問題**: 1時間更新では急激な変動に遅れる可能性があり、stale な手数料で LP が損失を被るリスク
- **対策**:
  ...
  - **Staleness ガード**:
    - 最終観測から2時間超過（MAX_STALENESS）: 手数料をMID_FEE（0.2%）以上に引き上げ
    - 6時間超過（CIRCUIT_BREAKER_STALENESS）: サーキットブレーカー発動、プール一時停止
  - **将来的な拡張（V3以降）**:
